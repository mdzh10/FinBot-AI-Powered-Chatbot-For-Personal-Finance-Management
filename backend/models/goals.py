...
class Goal(Base):
    ...
    goal_period = Column(Enum(GoalPeriodEnum), nullable=False)  # Frequency of the goal (monthly, yearly, etc.)
    spending_limit = Column(Float, nullable=False)  # Max allowed spending for the goal
    created_at = Column(DateTime, default=lambda: datetime.now(timezone.utc))  # Updated to use timezone-aware datetime
    updated_at = Column(DateTime, default=lambda: datetime.now(timezone.utc), onupdate=lambda: datetime.now(timezone.utc))  # Updated for timezone-aware datetime on update
